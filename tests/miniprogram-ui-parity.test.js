...
assert.match(homeScript, /reportService\.listMyReports/);
assert.match(homeScript, /removeStorageSync\("newhzLatestPreview"\)/);
...
assert.match(detail, /色号选择/);
assert.match(detail, /唇妆画法/);
assert.match(detail, /质地搭配/);
assert.match(detail, /presentation\.sharedContent\.shadeSelection/);
assert.match(detail, /presentation\.sharedContent\.lipMakeupMethod/);
assert.match(detail, /presentation\.sharedContent\.textureMatching/);
assert.match(detail, /product-color-swatch/);
assert.doesNotMatch(detail, /lipstick-brush-canvas/);
...
assert.match(upload, /value: "冷白皮"/);
assert.match(upload, /value: "冷黄皮"/);
assert.match(upload, /value: "暖黄皮"/);
assert.match(upload, /value: "黄黑皮"/);
assert.match(upload, /value: "100以内"/);
assert.match(upload, /value: "鹅蛋脸"/);
assert.match(upload, /value: "方脸"/);
assert.match(upload, /value: "菱形脸"/);
assert.match(upload, /value: "圆脸"/);
assert.match(upload, /faceShape: this\.data\.faceShape/);
assert.match(uploadView, /pref-title">脸型/);
assert.match(uploadView, /options\.faceShape/);
...
assert.doesNotMatch(reportScript, /renderLipstickBrush/);
...
assert.ok(fs.existsSync(path.join(__dirname, "..", "miniprogram/images/ui/lipstick-brush-mask.png")));
